...
const LibraryView: React.FC<LibraryViewProps> = ({ tracks, onRefresh }) => {
  ...
  const triggerCloudHarvest = async () => {
    ...
    addLog(">> CONNECTING TO DROPBOX...");
    try {
        const tracks = await harvestDropbox((msg) => addLog(`>> ${msg}`));
        
        if (tracks && tracks.length > 0) {
            addLog(`>> FOUND ${tracks.length} TRACKS`);
            setCloudIndex(tracks);
            setSelectedIds(new Set(tracks.map((t: Track) => t.id)));
  ...
  return (
    <div className="flex flex-col flex-1 space-y-6 overflow-hidden">
      ...
      <div className="bg-[#050507] rounded-xl sm:rounded-[2rem] border border-zinc-900 shadow-inner overflow-hidden flex flex-col relative">
        {isHarvesting && (
